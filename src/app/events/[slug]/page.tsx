...
import StreamFestivalOverview from "@/components/StreamFestivalOverview";
import StreamWorkshopGrid from "@/components/StreamWorkshopGrid";
...
export default async function EventDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isStreamFestival = event.slug === "stream-festival-2026";
  ...
  return (
    <article className="container-shell py-16">
      ...
      {isStreamFestival ? (
        <StreamFestivalOverview />
      ) : (
        <div className={`event-detail-media mt-8 max-w-3xl ${visual?.tone === "icon" ? "event-media-icon" : "event-media-photo"}`}>
          {visual ? (
            <Image
              alt=""
      ...
      {isStreamFestival ? <StreamWorkshopGrid /> : null}
    </article>
  );
}
